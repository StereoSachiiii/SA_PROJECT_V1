...
export default function AdminStallDesigner() {
    ...
    // State
    const [eventId, setEventId] = useState<number>(APP_CONFIG.DEMO_EVENT_ID); // Default to Demo Event
    ...
    useEffect(() => {
        const loadEventData = async () => {
            try {
                ...
                // Set map background from layoutConfig
                if (event.layoutConfig) {
                    try {
                        const config = JSON.parse(event.layoutConfig);
                        if (config.mapUrl) setMapUrl(config.mapUrl);
                    } catch (e) {
    ...
    const handleMapUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setLoading(true);
            // Upload to backend
            const response = await adminApi.uploadVenueMap(eventId, file);
            setMapUrl(response.url);
            setMessage("Map uploaded and saved to server.");
        } catch (err: any) {
            console.error("Upload failed", err);
            setMessage("Failed to upload map to server: " + (err.message || "Unknown error"));
            // Fallback to local preview for UX
            const objectUrl = URL.createObjectURL(file);
            setMapUrl(objectUrl);
        } finally {
            setLoading(false);
    ...
    const handleSave = async () => {
        try {
            setLoading(true);
            // Map DesignerStall back to Partial<EventStall>
            const payload: Partial<EventStall>[] = stalls.map(s => ({
                id: s.id,
                templateName: s.templateName,
                priceCents: s.priceCents,
                reserved: s.status === 'RESERVED' || s.status === 'SOLD',
                geometry: s.geometry
            }));

            await adminApi.saveLayout(eventId, payload);
