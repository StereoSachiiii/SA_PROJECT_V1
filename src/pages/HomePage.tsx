...
function HomePage() {
    ...
    const addGenreMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['genres', publisherId] })
            setNewGenre('')
        },
    })
    ...
    const handleAddGenre = (e: React.FormEvent) => {
        ...
        addGenreMutation.mutate({ publisherId, name: newGenre })
    }
            ...
            <section>
                ...
                <div className="flex flex-wrap gap-2">
                    {commonGenres.map((genre) => (
                        <button
                            key={genre}
                            onClick={() => addGenreMutation.mutate({ publisherId, name: genre })}
